...
- **UI-операции в amo и превью агент делает сам через браузер** (chrome-devtools MCP или скилл `agent-browser`): клики, чтение консоли/Network для CSP. **Основной дев-цикл — ИНЖЕКТ бандла `evaluate_script`'ом** (WIDGET_DEV_GUIDE §3, образец `~/amoai/widget/make-dev-inject.mjs`) — правка→инжект→проверка за секунды; перезаливка zip на каждую правку — запрещённая трата времени. Загрузка zip — редкое событие (первичная установка, финальный E2E): пробовать по `~/amoai/DEPLOY.md` §2 (форма «Мое приложение / Создать интеграцию → Загрузить архив» — приватная интеграция; **НЕ формы публикации в каталог маркетплейса**) с таймбоксом ~10 мин; не вышло / UI завис → zip оставить в `widget/`, в activity «готов, ожидает ручной загрузки человеком», статус `partial`, продолжать всё возможное инжектом — человек загрузит сам. Сессия `dzenteamdev.amocrm.ru` должна быть уже залогинена (оставляет пользователь); сессии нет → НЕ подбирать пароли, «непроверено живьём» по общему правилу.
...
- Загрузка zip: амоМаркет → «Мое приложение» → «Загрузить архив»; приватная интеграция БЕЗ модерации, активна сразу. Проверенный MCP-браузер-флоу: `~/amoai/DEPLOY.md` §2 «Путь A» (скрытый `input#input-upload-archive`). Ошибки манифеста amo показывает модалкой с точным текстом.
...
- **Линтер загрузчика** режет `console.*` (в т.ч. ссылки-колбэки `.catch(console.warn)`), `alert`/`confirm`/`prompt`. Перед zip: `node --check script.js` + `grep -cE 'console[.[(]|\b(confirm|alert|prompt)\(' script.js` = 0.
...
- **spec:** Каркас `amocrm-app/widget/`: `manifest.json` — widget-скелет по референсу Triggeron, `locale:['ru']`, `locations: ['settings','advanced_settings','lcard-0']` (DP/salesbot-блоки НЕ включать — добавятся в фазе 2, когда появится живой `webhook_url` моста), `settings.install_key` — точная форма: `settings: { install_key: { name: '<i18n-ключ>', type: 'text', required: false } }` (required:false — на PoC ключа ещё нет), `advanced.title`. `script.js` — vanilla AMD-модуль с минимальными `init/render/settings/onSave/advancedSettings` (все с `return true`); **`destroy` — пустой, ядро — синглтон `window.__dzenflow` с гардом в `init()`** (боевой факт: amo пересоздаёт инстанс на каждый SPA-переход); `advancedSettings()` рендерит заглушку «Автоматизации Dzen.Team» в рабочую область (`#work-area-<widget_code>`). Без `console.*`/`alert`/`confirm` в коде (линтер загрузчика). `i18n/ru.json` со всеми ключами манифеста. `images/` — заглушки логотипов **точно по размерам спеки из справочника** (logo_main 400×272 и др., иначе валидация). `build.sh` — упаковка zip без macOS-мусора (`zip -r -X`, исключить `.DS_Store`/`__MACOSX`), manifest.json в КОРНЕ архива; перед zip — `node --check script.js` + grep-проверка из справочника. Установку на dzenteamdev попробовать самому **по проверенному флоу `~/amoai/DEPLOY.md` §2 «Путь A»** (форма «Мое приложение / Создать интеграцию → Загрузить архив» — НЕ публикация в каталог; скрытый `input#input-upload-archive`, актуальность id сверить) — **таймбокс ~10 мин**: не вышло/зависло → zip готов, activity «ожидает ручной загрузки человеком», статус `partial`; заглушку `advancedSettings()` в это время проверить дев-инжектом (WIDGET_DEV_GUIDE §3).
...
- **verify:** zip собран; виджет установлен и активен на dzenteamdev; страница advanced_settings открывается, заглушка рендерится, консоль без наших ошибок. В activity зафиксировать: выданный amo `widget_code` и фактический URL страницы advanced_settings (вида `/settings/widgets/#<code>` или каким он окажется) — нужны W011/W013. Если установка агентом не удалась (UI не найден/завис) — это НЕ блокер: статус `partial`, zip в `widget/`, в activity точное описание, где застряло, + просьба человеку загрузить; следующая итерация после ручной установки добирает verify (widget_code, URL advanced_settings).
