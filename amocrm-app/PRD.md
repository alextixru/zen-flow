...
- Zip виджета грузится ТОЛЬКО руками через UI amo (API загрузки нет). Настройки, хранимые amo, стираются при деактивации/реактивации виджета.
...
- **spec:** Каркас `amocrm-app/widget/`: `manifest.json` — widget-скелет по референсу Triggeron, `locale:['ru']`, `locations: ['settings','advanced_settings','lcard-0']` (DP/salesbot-блоки НЕ включать — добавятся в фазе 2, когда появится живой `webhook_url` моста), `settings.install_key` (type text, required:false — на PoC ключа ещё нет), `advanced.title`. `script.js` — AMD-модуль с минимальными `init/render/settings/onSave/advancedSettings` (все с `return true`); `advancedSettings()` рендерит заглушку «Автоматизации Dzen.Team» в рабочую область (`#work-area-<widget_code>`). `i18n/ru.json` со всеми ключами манифеста. `images/` — заглушки логотипов (минимум, чтобы zip прошёл валидацию). `build.sh` — упаковка zip без macOS-мусора (`zip -r -X`, исключить `.DS_Store`/`__MACOSX`). Установить руками на dzenteamdev как приватный виджет; фактический путь загрузки zip в текущем UI amo зафиксировать в activity.
...
- **verify:** zip собран; виджет установлен и активен на dzenteamdev; страница advanced_settings открывается, заглушка рендерится, консоль без наших ошибок. Если UI загрузки приватного виджета не найден — ` — BLOCKED` (это предусловие всего проекта), с точным описанием, что искали и где.

### - [ ] W002 — Живая проверка CSP + размеры областей (открытые вопросы №1–2)
- **spec:** В `advancedSettings()` вставить `<iframe src="https://<frameable https-URL>" style="width:100%;height:<замер>px">` и снять на живом стенде: (а) режет ли CSP amo произвольный внешний iframe (консоль/Network — главный риск MVP; прецедент Sensei говорит «нет»); (б) фактические размеры work-area advanced_settings (px, скролл, поведение при ресайзе окна) и правой lcard-панели; (в) стабильность iframe при переходах между разделами amo и возврате. Для src годится любой https-ресурс без anti-framing заголовков (например, страница на GitHub Pages/нашем домене); мост ещё не существует. Результаты — числами и скринами в activity.
...
- **spec:** Поднять форк в EE-режиме по справочнику (Postgres пользователя + API 8080 + worker 8082 + cloudflared-туннель, `AP_FRONTEND_URL`=туннель). В Platform Settings: проверить `embeddingEnabled`, создать signing key (приватный ключ + kid → `bridge/.env`, файл вне git), добавить `https://dzenteamdev.amocrm.ru` в allowedEmbedOrigins через UI. Первый файл моста — `bridge/scripts/sign-jwt.ts`: подписывает JWT v3 (`externalProjectId:'32453394'`, `externalUserId` = id юзера amo, `firstName/lastName`, `exp` 1ч) — без сервера, одноразовый запуск. В виджете: `advancedSettings()` подключает `<script src="{туннель}/embed/<версия>.js">`, вызывает `configure({ instanceUrl: туннель, jwtToken: <вставлен руками перед сборкой, НЕ коммитить>, embedding: { containerId, locale:'ru', dashboard:{hideSidebar:true} } })`.
- **files:** `amocrm-app/bridge/{package.json,tsconfig.json,scripts/sign-jwt.ts,.env.example,.gitignore}`, `widget/script.js`.
- **pattern:** `research/embed-sdk.md` §3 (claims), §7 (чек-лист виджета).
- **verify:** живьём: конструктор открылся внутри advanced_settings на dzenteamdev; в форке автосоздан Project `32453394` и managed-юзер; reload страницы amo → повторный `configure()` отрабатывает, iframe не задваивается. В activity: точные env-переменные, URL туннеля, все грабли.
...
- **spec:** manifest: добавить `card_sdk` (требует объявленного `lcard-0` — уже есть). Вкладка в карточке сделки: (1) список последних ранов flow проекта — мост `GET /runs?install_key&account_id` → `GET {fork}/v1/flow-runs` (форму листинга сверить с контроллером); честное ограничение MVP: раны не фильтруются по конкретной сделке (ран не знает lead_id) — показываем последние раны проекта с пометкой в UI, `ponytail:` фильтр по сделке — upgrade (нужна конвенция передачи lead_id в flow); (2) кнопка «Запустить» у webhook-flow (список из `GET /flows` W014) → `POST /run {install_key, account_id, flow_id, lead_id}` на мосту → webhook-запуск с `{source:'amocrm_card', lead_id, manual:true}`. UI — vanilla JS/CSS в `widget-app.js`, размеры вкладки снять живьём.
...
- **spec:** manifest: `mobile: { frame_url: 'https://<мост>/mobile/card', color }`. Мост: `GET /mobile/card` — простая серверная HTML-страница (без SPA): статус последних ранов + подсказка «полный конструктор — в веб-версии». **Контракт не задокументирован (открытый вопрос №4 RESEARCH):** первым делом залогировать на мосту ВСЕ query-параметры и заголовки, которые amo передаёт в frame_url (какие id аккаунта/сделки/юзера доступны), зафиксировать в activity — от этого зависит, что страница может показать.
